...
test('fulfill [] find', async (t) => {
  ...
  const output = await apr.find(input.map(Number), async (v, i) => {
    await then(v);
    order.push(i);
    return v === 2;
  });
...
test('fulfill @@Iterator find', async (t) => {
  ...
  const output = await apr.find(getIttr(), async (v, i) => {
    await then(`${v}${v}`);
    order.push(i);
    return i === 2;
  });
...
test('fail [] find', async (t) => {
  t.throws(apr.find([1, 2, 3, 4], async (v, i) => {
    if (i > 2) {
...
test('fail @@Iterator find', async (t) => {
  t.throws(apr.find(getIttr(), async (v, i) => {
    if (i > 2) {
      throw new Error('expected error');
    }
...
test('fulfill [] findSeries', async (t) => {
  ...
  const output = await apr.findSeries(input.map(Number), async (v, i) => {
    await then(v);
    order.push(i);
    return v === 2;
  });
...
test('fulfill @@Iterator findSeries', async (t) => {
  ...
  const output = await apr.findSeries(getIttr(), async (v, i) => {
    await then(`${v}${v}`);
    order.push(i);
    return i === 2;
  });
...
test('fail [] findSeries', async (t) => {
  t.throws(apr.findSeries([1, 2, 3, 4], async (v, i) => {
    if (i > 2) {
      throw new Error('expected error');
    }
...
test('fail @@Iterator findSeries', async (t) => {
  t.throws(apr.findSeries(getIttr(), async (v, i) => {
    if (i > 2) {
      throw new Error('expected error');
    }
